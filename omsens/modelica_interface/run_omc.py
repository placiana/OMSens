...
#Mine
import omsens.settings.gral_settings as gral_settings
...
def runMosScript(script_path):
    ...
    output = omsens.filesystem.files_aux.callCMDStringInPath(command,script_folder_path)
...
def writeOMCLog(log_str, output_path):
    ...
    omsens.filesystem.files_aux.writeStrToFile(final_str,output_path)
    return 0
def removeTemporaryFiles(folder_path):
    regex = '.*\.(c|o|h|makefile|log|libs|json)$'
    omsens.filesystem.files_aux.removeFilesWithRegexAndPath(regex,folder_path)
